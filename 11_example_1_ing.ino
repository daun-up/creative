...
#define _DIST_MAX 360.0   // maximum distance to be measured (unit: mm)  // 180 에서 360 사이를 과제에선 범위로 제시
...
void loop() {
  ...
  if (dist_ema <= _TARGET_LOW) {
    ...
    double i = ( dist_ema - _DIST_MIN )/(_TARGET_HIGH - _TARGET_LOW)*(_DUTY_MAX - _DUTY_MIN) + _DUTY_MIN ;
